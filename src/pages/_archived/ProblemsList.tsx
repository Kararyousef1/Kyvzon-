...
import { incidentService } from '../../services/sdk';
...
export default function ProblemsList({ isHR: isHRProp = false }: ProblemsListProps) {
  ...
  const fetchProblems = useCallback(async () => {
    ...
    try {
      // ★★★ إصلاح 0338 — كان هنا استعلام مباشر على `incidents` يبني:
      //     employeeName: isHR ? d.employee_name || 'موظف' : user.full_name
      //   **بلا أي فحص لـ`is_anonymous`**. فالبلاغ «المجهول» يكشف اسم
      //   صاحبه لمسؤول الموارد — إخلال بوعد صريح قد يُعرّض المُبلِّغ
      //   للانتقام. مُثبَت على Postgres: employee_name = 'أحمد الموظف'
      //   رغم is_anonymous = TRUE.
      //
      //   الإخفاء صار في القاعدة: الاسم والمعرّف والقسم، والبحث بالاسم
      //   لا يشمل المجهول (وإلا كُشفت الهوية بالاستنتاج).
      // ★ فرعان صريحان لا `in`: الأخير لا يُضيّق النوع في الاتحاد،
      //   والصندوق يحمل حقولاً لا وجود لها في شاشة الموظف.
      const mapped: Problem[] = isHR
        ? (await incidentService.hrInbox({ limit: 200 })).rows.map((d) => ({
            id: d.id,
            title: d.title || 'بلاغ',
            ...
            // ★★ يعود null للمجهول — الإخفاء من القاعدة لا هنا
            employeeId: d.employeeId ?? '',
            employeeName: d.reporter,
        ...
        : (await incidentService.myIncidents({ limit: 200 })).rows.map((d) => ({
            id: d.id,
            title: d.title || 'بلاغ',
            description: d.description || '',
            category: d.category || 'other',
