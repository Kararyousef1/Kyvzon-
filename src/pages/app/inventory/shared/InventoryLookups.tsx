...
function useLookup(table: string, mapper: (r: Record<string, unknown>) => LookupOption, select = '*', deps: unknown[] = []) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!mounted) return;
        if (error) setRows([]);
        else setRows(((data || []) as unknown as Record<string, unknown>[]).map(mapper));
      } finally {
...
export function ItemLookup({ label = 'الصنف', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_items', r => ({ id: String(r.id), code: String(r.item_code || ''), name: String(r.name_ar || ''), meta: String(r.base_uom || '') }), 'id,item_code,name_ar,base_uom,status');
...
export function WarehouseLookup({ label = 'المستودع', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_warehouses', r => ({ id: String(r.id), code: String(r.warehouse_code || ''), name: String(r.name_ar || ''), meta: String(r.warehouse_type || '') }), 'id,warehouse_code,name_ar,warehouse_type,status');
...
export function LocationLookup({ label = 'الموقع', value, onChange, required, warehouseId }: Omit<InventoryLookupProps, 'options'|'loading'> & { warehouseId?: string }) {
  const { options, loading } = useLookup('inventory_locations', r => ({ id: String(r.id), code: String(r.location_code || ''), name: String(r.barcode || ''), meta: String(r.location_type || ''), warehouseId: String(r.warehouse_id || '') }), 'id,warehouse_id,location_code,barcode,location_type,status', [warehouseId]);
...
export function DockLookup({ label = 'الرصيف', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_docks', r => ({ id: String(r.id), code: String(r.dock_code || ''), name: String(r.dock_type || ''), meta: String(r.status || '') }), 'id,dock_code,dock_type,status');
...
export function AsnLookup({ label = 'ASN', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_asns', r => ({ id: String(r.id), code: String(r.asn_number || ''), name: String(r.status || ''), meta: r.expected_arrival_at ? new Date(String(r.expected_arrival_at)).toLocaleDateString('ar-SA') : '' }), 'id,asn_number,status,expected_arrival_at');
...
export function ReceivingSessionLookup({ label = 'جلسة الاستلام', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_receiving_sessions', r => ({ id: String(r.id), code: String(r.session_number || ''), name: String(r.status || ''), meta: String(r.source_type || '') }), 'id,session_number,status,source_type');
...
export function RmaLookup({ label = 'RMA', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_rmas', r => ({ id: String(r.id), code: String(r.rma_number || ''), name: String(r.status || ''), meta: String(r.reason_code || '') }), 'id,rma_number,status,reason_code');
  return <InventoryLookup label={label} value={value} onChange={onChange} required={required} options={options} loading={loading} />;
}

export function RmaLineLookup({ label = 'سطر RMA', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_rma_lines', r => ({ id: String(r.id), code: String(r.id || '').slice(0,8), name: `Qty ${String(r.expected_qty || '')}`, meta: String(r.status || '') }), 'id,expected_qty,status');
...
export function SupplierLookup({ label = 'المورد', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('suppliers', r => ({ id: String(r.id), code: String(r.supplier_code || r.code || r.id || ''), name: String(r.name_ar || r.name || r.company_name || '') }), '*');
...
export function WorkerLookup({ label = 'الموظف', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('profiles', r => ({ id: String(r.id), code: String(r.email || r.id || ''), name: String(r.full_name || ''), meta: String(r.role || '') }), 'id,email,full_name,role');
...
export function PickOrderLookup({ label = 'أمر السحب', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_pick_orders', r => ({ id: String(r.id), code: String(r.pick_order_number || ''), name: String(r.status || ''), meta: String(r.priority || '') }), 'id,pick_order_number,status,priority');
  return <InventoryLookup label={label} value={value} onChange={onChange} required={required} options={options} loading={loading} />;
}
export function PickTaskLookup({ label = 'مهمة السحب', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_pick_tasks', r => ({ id: String(r.id), code: String(r.task_number || ''), name: String(r.status || ''), meta: String(r.required_qty || '') }), 'id,task_number,status,required_qty');
  return <InventoryLookup label={label} value={value} onChange={onChange} required={required} options={options} loading={loading} />;
}
export function PickWaveLookup({ label = 'موجة السحب', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_pick_waves', r => ({ id: String(r.id), code: String(r.wave_number || ''), name: String(r.status || ''), meta: String(r.wave_type || '') }), 'id,wave_number,status,wave_type');
  return <InventoryLookup label={label} value={value} onChange={onChange} required={required} options={options} loading={loading} />;
}
export function PickListLookup({ label = 'قائمة السحب', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_pick_lists', r => ({ id: String(r.id), code: String(r.pick_list_number || ''), name: String(r.status || ''), meta: String(r.route_algorithm || '') }), 'id,pick_list_number,status,route_algorithm');
  return <InventoryLookup label={label} value={value} onChange={onChange} required={required} options={options} loading={loading} />;
}

export function PackageLookup({ label = 'الطرد', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_packages', r => ({ id: String(r.id), code: String(r.package_number || ''), name: String(r.status || ''), meta: String(r.package_barcode || '') }), 'id,package_number,package_barcode,status');
  return <InventoryLookup label={label} value={value} onChange={onChange} required={required} options={options} loading={loading} />;
}
export function ShipmentLookup({ label = 'الشحنة', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_shipments', r => ({ id: String(r.id), code: String(r.shipment_number || ''), name: String(r.status || ''), meta: String(r.tracking_number || '') }), 'id,shipment_number,status,tracking_number');
  return <InventoryLookup label={label} value={value} onChange={onChange} required={required} options={options} loading={loading} />;
}
export function ManifestLookup({ label = 'Manifest', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_loading_manifests', r => ({ id: String(r.id), code: String(r.manifest_number || ''), name: String(r.status || ''), meta: String(r.truck_number || '') }), 'id,manifest_number,status,truck_number');
  return <InventoryLookup label={label} value={value} onChange={onChange} required={required} options={options} loading={loading} />;
}
export function CarrierLookup({ label = 'الناقل', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_carriers', r => ({ id: String(r.id), code: String(r.carrier_code || ''), name: String(r.name_ar || ''), meta: String(r.provider || '') }), 'id,carrier_code,name_ar,provider');
  return <InventoryLookup label={label} value={value} onChange={onChange} required={required} options={options} loading={loading} />;
}

export function CountPlanLookup({ label = 'خطة الجرد', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_cycle_count_plans', r => ({ id: String(r.id), code: String(r.plan_number || ''), name: String(r.status || ''), meta: String(r.plan_type || '') }), 'id,plan_number,status,plan_type');
  return <InventoryLookup label={label} value={value} onChange={onChange} required={required} options={options} loading={loading} />;
}
export function CountTaskLookup({ label = 'مهمة العد', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_count_tasks', r => ({ id: String(r.id), code: String(r.task_number || ''), name: String(r.status || ''), meta: `Round ${String(r.count_round || '')}` }), 'id,task_number,status,count_round');
  return <InventoryLookup label={label} value={value} onChange={onChange} required={required} options={options} loading={loading} />;
}
export function CountLineLookup({ label = 'سطر العد', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_count_task_lines', r => ({ id: String(r.id), code: String(r.id || '').slice(0,8), name: String(r.counted_qty ?? 'غير معدود'), meta: String(r.system_qty ?? '') }), 'id,counted_qty,system_qty');
  return <InventoryLookup label={label} value={value} onChange={onChange} required={required} options={options} loading={loading} />;
}
...
export function ReturnReceiptLineLookup({ label = 'سطر استلام المرتجع', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_return_receipt_lines', r => ({ id: String(r.id), code: String(r.id || '').slice(0,8), name: String(r.status || ''), meta: String(r.received_qty || '') }), 'id,status,received_qty');
  return <InventoryLookup label={label} value={value} onChange={onChange} required={required} options={options} loading={loading} />;
}
export function ReturnAssessmentLookup({ label = 'تقييم المرتجع', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_return_condition_assessments', r => ({ id: String(r.id), code: `${String(r.condition_grade || '')}-${String(r.id || '').slice(0,8)}`, name: String(r.recommended_disposition || ''), meta: String(r.defect_type || '') }), 'id,condition_grade,recommended_disposition,defect_type');
  return <InventoryLookup label={label} value={value} onChange={onChange} required={required} options={options} loading={loading} />;
}
export function ReturnDispositionTaskLookup({ label = 'مهمة التوجيه', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_return_disposition_tasks', r => ({ id: String(r.id), code: String(r.task_number || ''), name: String(r.disposition || ''), meta: String(r.status || '') }), 'id,task_number,disposition,status');
  return <InventoryLookup label={label} value={value} onChange={onChange} required={required} options={options} loading={loading} />;
}
export function ProductionReturnLookup({ label = 'إرجاع الإنتاج', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_production_returns', r => ({ id: String(r.id), code: String(r.production_return_number || ''), name: String(r.status || ''), meta: String(r.reason_code || '') }), 'id,production_return_number,status,reason_code');
  return <InventoryLookup label={label} value={value} onChange={onChange} required={required} options={options} loading={loading} />;
}

export function LaborTaskLookup({ label = 'مهمة العمالة', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_labor_dispatch_tasks', r => ({ id: String(r.id), code: String(r.task_number || ''), name: String(r.task_type || ''), meta: String(r.status || '') }), 'id,task_number,task_type,status');
  return <InventoryLookup label={label} value={value} onChange={onChange} required={required} options={options} loading={loading} />;
}
export function LaborInterleavingSuggestionLookup({ label = 'اقتراح الدمج', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_labor_task_interleaving_suggestions', r => ({ id: String(r.id), code: String(r.id || '').slice(0,8), name: String(r.status || ''), meta: String(r.estimated_minutes_saved || '') }), 'id,status,estimated_minutes_saved');
  return <InventoryLookup label={label} value={value} onChange={onChange} required={required} options={options} loading={loading} />;
}
export function IncentiveProgramLookup({ label = 'برنامج الحوافز', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_labor_incentive_programs', r => ({ id: String(r.id), code: String(r.program_name || ''), name: String(r.status || ''), meta: `${String(r.period_start || '')} → ${String(r.period_end || '')}` }), 'id,program_name,status,period_start,period_end');
  return <InventoryLookup label={label} value={value} onChange={onChange} required={required} options={options} loading={loading} />;
}

export function AlertLookup({ label = 'التنبيه', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_analytics_alerts', r => ({ id: String(r.id), code: String(r.title || ''), name: String(r.severity || ''), meta: String(r.status || '') }), 'id,title,severity,status');
  return <InventoryLookup label={label} value={value} onChange={onChange} required={required} options={options} loading={loading} />;
}
export function ReportRunLookup({ label = 'تشغيل التقرير', value, onChange, required }: Omit<InventoryLookupProps, 'options'|'loading'>) {
  const { options, loading } = useLookup('inventory_periodic_report_runs', r => ({ id: String(r.id), code: String(r.report_number || ''), name: String(r.report_type || ''), meta: String(r.delivery_status || '') }), 'id,report_number,report_type,delivery_status');
  return <InventoryLookup label={label} value={value} onChange={onChange} required={required} options={options} loading={loading} />;
}
