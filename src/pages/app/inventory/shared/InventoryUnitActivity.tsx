...
export function InventoryUnitActivity({unit}:{unit:UnitKey}){
  const [rows,setRows]=useState<Activity[]>([]);
  useEffect(()=>{let ok=true; supabase.from('inventory_unit_activity').select('*').eq('unit_key',unit).limit(8).then(({data})=>{if(ok)setRows(data||[])}); return()=>{ok=false};},[unit]);
