/**
 * ═════════════════════════════════════════════════════════════════════════
 *  CrmSidebar — الشريط الجانبي لبوابة CRM
 *
 *  يعرض لوحة البوابة + الوحدات الست. غير الجاهزة تُعرَض بشارة "قريباً".
 *  التصميم: بهوية بوابة CRM (cyan/أزرق).
 * ═════════════════════════════════════════════════════════════════════════
 */

import { useNavigate, useLocation } from 'react-router-dom';
import { ChevronRight, LogOut, Briefcase, CheckCircle2 } from 'lucide-react';
import { useAuthStore, useUIStore } from '../../core/stores';
import { getUserDisplayName } from '../../utils/userUtils';
import { CRM_NAV } from './crmCatalog';
...
export default function CrmSidebar() {
  ...
  const isActive = (path: string) =>
    location.pathname === path || (path !== '/app/crm' && location.pathname.startsWith(path + '/'));
            ...
            <div className="min-w-0">
              <div
                style={{
                  ...
                  background: 'linear-gradient(135deg, #ffffff 0%, #cffafe 70%, #22d3ee 100%)',
              ...
              <p className="text-white/70 text-xs truncate">بوابة CRM</p>
            </div>
          )}
        </div>
        {sidebarOpen && (
        ...
        <div className="p-4 flex-shrink-0 border-b border-slate-100 bg-gradient-to-br from-cyan-50 to-white">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-slate-500 text-xs truncate mt-0.5">{user?.position || 'فريق المبيعات'}</p>
            </div>
          </div>
        </div>
      )}
