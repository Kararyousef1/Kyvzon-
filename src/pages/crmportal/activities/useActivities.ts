/**
 * useActivities — خطافات بيانات الوحدة 3 (الأنشطة والأتمتة).
 */
import { useCallback, useEffect, useState } from 'react';
import {
  crmTaskService, crmSequenceService, crmAutomationService, crmAssignmentService,
  type CrmTask, type CrmSequence, type CrmAutomationRule, type CrmAssignmentRule,
} from '../../../services/sdk';
...
export const useTasks = () => useAsync<CrmTask[]>(() => crmTaskService.listTasks(), []);
export const useSequences = () => useAsync<CrmSequence[]>(() => crmSequenceService.listSequences(), []);
export const useAutomationRules = () => useAsync<CrmAutomationRule[]>(() => crmAutomationService.listRules(), []);
export const useAssignmentRules = () => useAsync<CrmAssignmentRule[]>(() => crmAssignmentService.listRules(), []);
