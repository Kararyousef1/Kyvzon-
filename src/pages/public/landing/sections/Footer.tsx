...
export function Footer({ landingConfig, publicConfig }: { landingConfig?: Partial<LandingConfig> | null; publicConfig?: PublicSiteConfig }) {
  ...
  return (
    <footer className="relative kv-footer-dark">
      {/* خط ضوئي متدرج يفصل التذييل */}
      <div
        className="absolute top-0 left-0 right-0 h-px"
        style={{ background: 'linear-gradient(90deg, transparent, rgba(12,141,231,0.35), rgba(0,109,197,0.2), transparent)' }}
        aria-hidden="true"
      />
      {/* توهج سفلي خافت */}
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[600px] h-[200px] rounded-full bg-sky-400/[0.06] blur-[100px] pointer-events-none" aria-hidden="true" />
      ...
      <div className="relative max-w-7xl mx-auto px-4 md:px-8 pt-16 pb-10">
        <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-10 pb-12">
          ...
          <div className="lg:col-span-2">
            <div className="flex items-center gap-2.5 mb-4">
              <div className="relative">
                <div className="absolute -inset-1 rounded-xl blur-md opacity-50" style={{ background: 'linear-gradient(135deg, #0c8de7, #0057a0)' }} aria-hidden="true" />
                <div className="relative w-9 h-9 rounded-lg bg-gradient-to-br from-sky-500 to-blue-600 flex items-center justify-center text-white font-black text-sm shadow-lg shadow-sky-500/40">K</div>
              </div>
            ...
            <p style={{ color: 'rgba(203,213,225,0.7)', fontSize: '0.875rem', lineHeight: '1.85', maxWidth: '22rem' }}>
          ...
          <div>
            ...
            <ul className="flex flex-col gap-3">
              {productLinks.map((l) => (
                <li key={l.label}>
                  <button onClick={l.action} style={{ color: 'rgba(203,213,225,0.75)', fontSize: '0.85rem' }} className="hover:text-white hover:translate-x-0.5 transition-all text-start">
                    {l.label}
                  </button>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="flex flex-col gap-3">
              {companyLinks.map((l) => (
                <li key={l.label}>
                  <button onClick={() => navigate(l.href)} style={{ color: 'rgba(203,213,225,0.75)', fontSize: '0.85rem' }} className="hover:text-white hover:translate-x-0.5 transition-all text-start">{l.label}</button>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="flex flex-col gap-3">
              {resourceLinks.map((l) => (
                <li key={l.label}>
                  <button onClick={() => ('href' in l && l.href ? navigate(l.href) : l.action?.())} style={{ color: 'rgba(203,213,225,0.75)', fontSize: '0.85rem' }} className="hover:text-white hover:translate-x-0.5 transition-all text-start">{l.label}</button>
                </li>
              ))}
            </ul>
          </div>
        ...
        >
          ...
          <div className="flex items-center gap-2 order-2 px-3 py-1.5 rounded-full" style={{ color: 'var(--kv-text-muted)', fontSize: '0.75rem', fontWeight: 700, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)' }}>
            🇮🇶 {t('footer_made_in')}
          </div>
          <div className="flex items-center gap-5 order-1 md:order-3">
            {legalLinks.map((l) => (
              <button key={l.label} onClick={() => navigate(l.href)} style={{ color: 'var(--kv-text-muted)', fontSize: '0.75rem' }} className="hover:text-white/70 transition-colors">
                {l.label}
              </button>
            ))}
          </div>
