...
export interface MrpBomLineRecord { id:string; tenant_id:string; bom_version_id:string; line_no:number; component_item_id:string; quantity_per:number; scrap_percent:number; net_quantity:number; material_type:string; supply_type:string; created_at:string; }
export interface MrpEcrRecord { id:string; tenant_id:string; ecr_number:string; reason:string; proposed_change:string; status:string; created_at:string; }
export interface MrpEcoRecord { id:string; tenant_id:string; eco_number:string; status:string; effectivity_date?:string|null; created_at:string; }
export interface MrpBomExplosionRunRecord { id:string; tenant_id:string; run_number:string; demand_quantity:number; status:string; created_at:string; }
export interface MrpBomAvailabilityCheckRecord { id:string; tenant_id:string; check_number:string; demand_quantity:number; can_build:boolean; shortage_count:number; created_at:string; }
export interface MrpBomImportBatchRecord { id:string; tenant_id:string; batch_number:string; file_name?:string|null; status:string; created_at:string; }
export interface MrpBomExportRequestRecord { id:string; tenant_id:string; request_number:string; export_type:string; status:string; requested_at:string; }
...
export class MrpBomExplosionService extends BaseService<MrpBomExplosionRunRecord>{constructor(){super('mrp_bom_explosion_runs')} async explode(versionId:string,qty:number):Promise<string>{const{data,error}=await supabase.rpc('explode_mrp_bom',{p_bom_version_id:versionId,p_demand_quantity:qty}); if(error) throw new Error(error.message); return data as string;}}
export class MrpBomAvailabilityService extends BaseService<MrpBomAvailabilityCheckRecord>{constructor(){super('mrp_bom_availability_checks')} async check(versionId:string,qty:number):Promise<string>{const{data,error}=await supabase.rpc('check_mrp_bom_availability',{p_bom_version_id:versionId,p_demand_quantity:qty}); if(error) throw new Error(error.message); return data as string;}}
export class MrpBomImportBatchService extends BaseService<MrpBomImportBatchRecord>{constructor(){super('mrp_bom_import_batches')} async createBatch(input:Row):Promise<string>{const{data,error}=await supabase.rpc('create_mrp_bom_import_batch',{p_file_name:input.file_name,p_file_url:input.file_url||null,p_import_type:input.import_type||'excel'}); if(error) throw new Error(error.message); return data as string;}}
export class MrpBomExportRequestService extends BaseService<MrpBomExportRequestRecord>{constructor(){super('mrp_bom_export_requests')} async request(versionId:string,exportType='excel'):Promise<string>{const{data,error}=await supabase.rpc('request_mrp_bom_export',{p_bom_version_id:versionId,p_export_type:exportType}); if(error) throw new Error(error.message); return data as string;}}
class MrpBomAnalyticsService{async dashboard(){const{data,error}=await supabase.from('mrp_bom_dashboard').select('*').limit(10); if(error) throw new Error(error.message); return data||[]} async kpis(){const{data,error}=await supabase.from('mrp_bom_kpis').select('*').limit(10); if(error) throw new Error(error.message); return data||[]} async tree(){const{data,error}=await supabase.from('mrp_bom_tree').select('*').limit(500); if(error) throw new Error(error.message); return data||[]} async eco(){const{data,error}=await supabase.from('mrp_eco_dashboard').select('*').limit(100); if(error) throw new Error(error.message); return data||[]} async explosion(){const{data,error}=await supabase.from('mrp_bom_explosion_summary').select('*').limit(100); if(error) throw new Error(error.message); return data||[]} async shortages(){const{data,error}=await supabase.from('mrp_bom_availability_shortages').select('*').limit(100); if(error) throw new Error(error.message); return data||[]}}
export const mrpBomHeaderService=new MrpBomHeaderService();
export const mrpBomVersionService=new MrpBomVersionService();
export const mrpBomLineService=new MrpBomLineService();
export const mrpEcrService=new MrpEcrService();
export const mrpEcoService=new MrpEcoService();
export const mrpBomExplosionService=new MrpBomExplosionService();
export const mrpBomAvailabilityService=new MrpBomAvailabilityService();
export const mrpBomImportBatchService=new MrpBomImportBatchService();
export const mrpBomExportRequestService=new MrpBomExportRequestService();
export const mrpBomAnalyticsService=new MrpBomAnalyticsService();
